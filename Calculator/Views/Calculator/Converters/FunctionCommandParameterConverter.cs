...
{
    public class FunctionCommandParameterConverter : IMultiValueConverter
    {
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            if (!values.TryIndex(1, out FunctionType functionType))
            {
                throw new IndexOutOfRangeException($@"The following type was not defined: {typeof(FunctionType)} - {nameof(functionType)}");
            }

            return new FunctionCommandParameter(userInteractionType, functionType);
        }
